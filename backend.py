...
def search_similar_chunks(user_id: int, query: str, top_k=5):
    ...
    with engine.connect() as conn:
        result = conn.execute(text("""
            ...
            LIMIT :top_k
        """), {"uid": user_id, "embedding": query_embedding, "top_k": top_k})
...
async def telegram_webhook(request: Request):
    ...
    if text.startswith("/start"):
        ...
        if len(parts) < 2:
            ...
            return {"ok": True}
        token = parts[1]
        user = db.execute(text("SELECT * FROM users WHERE token=:t"), {"t": token}).fetchone()
        if not user:
            send_message(chat_id, "Invalid token.")
            return {"ok": True}
        db.merge({"chat_id": chat_id, "user_id": user.id})
        db.commit()
        send_message(chat_id, f"Connected to {user.username}. How can I help?")
        return {"ok": True}

    session = db.execute(text("SELECT * FROM telegram_sessions WHERE chat_id=:cid"), {"cid": chat_id}).fetchone()
    if not session:
