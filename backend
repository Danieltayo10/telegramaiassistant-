...
def search_similar_chunks(client_id: int, query: str, top_k=5):
...
def generate_answer(client_id: int, user_message: str):
    chunks = search_similar_chunks(client_id, user_message)
...
async def telegram_webhook(request: Request):
    ...
    if text.startswith("/start"):
        ...
        db.merge(TelegramSession(chat_id=chat_id, client_id=client.id))
        db.commit()
        send_message(chat_id, f"Connected to {client.name}. How can I help?")
        return {"ok": True}
    ...
    reply = generate_answer(session.client_id, text)
